...
import { extractDeploymentNeeds } from "../cloud/deployment-needs.extractor.js";
import { extractModernizationNeeds } from "../modernization/modernization-needs.extractor.js";
...
async function run(input: BusinessDiscoveryInput): Promise<DiscoveryResult> {
  ...
  const deploymentNeeds = extractDeploymentNeeds(parsed, allSystems);
  const modernizationNeeds = extractModernizationNeeds(parsed, allSystems);
